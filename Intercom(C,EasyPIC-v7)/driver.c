...
void SetRST(unsigned char value)
{
	if (value)
		RF5_1
	else
...
void SetRW(unsigned char value)
{
	if (value)
		RF1_1
	else
...
void SetRS(unsigned char value)
{
	if (value)
		RF0_1
	else
...
void SetCS2(unsigned char value)
{
	if (value)
		RB5_1
	else
...
void SetCS1(unsigned char value)
{
	if (value)
		RB4_1
	else
...
{
	TRISFbits.TRISF5=0;//LCD_RST output
	TRISFbits.TRISF4=0;//LCD_E output
	TRISFbits.TRISF1=0;//LCD_RW output
	TRISFbits.TRISF0=0;//LCD_DI output
	ADPCFGbits.PCFG5=1;
	TRISBbits.TRISB5=0;//LCD_CS2 output				
	ADPCFGbits.PCFG4=1;
	TRISBbits.TRISB4=0;//LCD_CS1 output
...
{
	SetE(1);		/* Strobe */
  //	LcdDelay(1);//////////works even without him
	SetE(0);
}
...
/*---------------------------------------------------
screen scrolling, where is startline set
this side goes up down to the setted line
accepts value 0-64
------------------------------------------------*/

void LcdSelectStartline(unsigned char start_line)
{
	LcdInstructionWrite (START_LINE | start_line);
}
...
unsigned char LcdDataRead (void)
{
	...
	SetRW(1);      	/* read mode */
	//LcdDelay(5);
	strobe_data();//double strobe is good
	//LcdDelay(5);
	SetE(1);
	//LcdDelay(5);
	temp=ReadLcdData();
...
/*-------------------------------------------------------------------------------
writes one character when over and goes back on the last
-------------------------------------------------------------------------------*/
void Glcd_PutChar (char AskiKod)
{
unsigned char trm;

if ((cursorX+FONT_LENGHT)>127)
	{
	cursorY++;
	if (cursorY>7)
		cursorY=0;
...
/*-------------------------------------------------------------------------------
writes one character on displaywhen new line overflow and back on 0 when last over
-------------------------------------------------------------------------------*/
...
for (trm=0;trm<8;trm++)
	...
	LcdInstructionWrite (PAGE_ADRESS | (cursorY+1));
if (cursorX<64)
	{
	LcdSelectSide(LEFT);
	LcdInstructionWrite (Y_ADRESS | cursorX);
	side=LEFT;
	}
else if(cursorX<128)
	{
	LcdSelectSide(RIGHT);
	LcdInstructionWrite (Y_ADRESS | (cursorX-64));
	side=RIGHT;
	}
	LcdDataWrite(fontBIG[(((AsciiCode-32)*16)+trm+8)]);
...
/*-------------------------------------------------------------------------------
draws greed on display
-------------------------------------------------------------------------------*/
void GLCD_ShowGreed(unsigned char space_greeda) 
{
unsigned char x_greed,y_greed,divided;

divided=space_greed/pixel_rectangularity_proportion;

for (x_greed=0;x_greed<127;x_greed=x_greed+3)
	for (y_greed=divided;y_greed<64-divided;y_greed=y_greed+divided)
		{
		LcdSetDot (x_greed,y_greed);
		}

for (x_greed=(space_greeda-2);x_greed<124;x_greed=x_greed+space_greeda)
